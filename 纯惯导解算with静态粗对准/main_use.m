clear;
format long;
Answer=fopen('Data1_PureINS.bin');
fid=fopen('Data1.bin');
FidResult=fopen('初始航向角用老师给的初始横滚角和初始俯仰角用静态粗对准得到的','wt');
Eulerk_1=[0.0107951084511778/180*pi;-2.14251290749072/180*pi;-76.798410598443212/180*pi];
%q_bk_12nk_1=RotationToQuater(Eulerk_1);
q_bk_12nk_1=EulerToQuater(Eulerk_1);
C_bk_12nk_1=EulerToDCM(Eulerk_1);
CCC=QuaterToDCM(q_bk_12nk_1);
BLHk_1=[23.1373950708/180*pi;113.3713651222/180*pi;2.175];
...
while(~feof(fid))
    ...
    if(size(x)==0)
        ...
        delta_theta_k_1=[x(2,1);x(3,1);x(4,1)];
        delta_v_k_1=[x(5,1);x(6,1);x(7,1)];%开始没把这速度读进来
        tk_1=x(1,1);
    else if((x(1,1)>91620))
        %姿态更新
        ...
        gl2n=[0;0;gl];
        deltavg2nk=(gl2n-cross(2*wie2nk_1+wen2nk_1,vnk_1))*0.005;
        
        t0=cross(delta_theta_k,delta_v_k);
        t1=cross(delta_theta_k_1,delta_v_k);
        t2=cross(delta_v_k_1,delta_theta_k);
        ...
        BLHk=BLHk_1;
        BLHk(3,1)=BLHk_1(3,1)-1/2*(vnk_1(3,1)+vnk(3,1))*0.005;
        h_ave=0.5*(BLHk_1(3,1)+BLHk(3,1));
        BLHk(1,1)=BLHk_1(1,1)+0.5*(vnk_1(1,1)+vnk(1,1))/(RMk_1+h_ave)*0.005;
        phi_ave=0.5*(BLHk_1(1,1)+BLHk(1,1));
        BLHk(2,1)=BLHk_1(2,1)+0.5*(vnk_1(2,1)+vnk(2,1))/((RNk_1+h_ave)*cos(phi_ave))*0.005;
        %disp(x(1,1));
